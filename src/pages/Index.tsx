...
const Index = () => {
  ...
  useEffect(() => {
    if (activeSection && contentRef.current) {
      ...
      // Focus the container element (not the heading) to avoid text selection/highlight.
      // Use a tiny timeout to ensure the rendered section DOM is ready.
      setTimeout(() => {
        if (!contentRef.current) return;
        // Make sure the container is programmatically focusable and then focus it.
        if (contentRef.current.getAttribute("tabindex") !== "-1") {
          contentRef.current.setAttribute("tabindex", "-1");
        }
        // Focus without causing extra scrolling
        (contentRef.current as HTMLElement).focus({ preventScroll: true });
      }, 50);
    }
  }, [activeSection]);
  ...
  return (
    <div className="min-h-screen bg-background relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl relative z-10">
        ...
        <div
          ref={contentRef}
          // allow the container to be programmatically focused (tabindex -1)
          tabIndex={-1}
