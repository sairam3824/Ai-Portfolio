    ...
    return (
        <BentoCard className="col-span-12 md:col-span-7" ghostChar="B" delay={600}>
            <div className="flex items-center justify-between mb-4">
                <h3 className="text-xl font-bold text-gray-800 tracking-tight">Latest {WRITING_LABEL}</h3>
                <Link
            ...
            <div className="flex flex-col gap-3">
                {latestPosts.map((post) => (
                    <Link
                        key={post.id}
                        to={post.externalLink ? post.externalLink : getWritingPath(post.id)}
                    ...
                    >
                        <div className="w-9 h-9 rounded-2xl bg-gradient-to-br from-indigo-500 to-purple-500 text-white flex items-center justify-center shadow-sm shadow-indigo-500/20">
                            <BookOpen className="w-4 h-4" />
                        </div>
                        <h4 className="text-lg font-bold text-gray-800 leading-snug line-clamp-2">{post.title}</h4>
